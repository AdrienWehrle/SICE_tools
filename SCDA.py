...
if multi_proc==True:
    ...
    def multiproc_SCDA(k):
        #saving profile metadata only for the first iteration
        if k==0:
            profile=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'r_TOA_S1.tif').profile
        
        #calibrating R16
        R16=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'r_TOA_S5.tif')
        radiometric_calibration(R16=R16,scene=scenes[k])
        
        #loading inputs
        R550=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'r_TOA_S1.tif').read(1)
        R16=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'r_TOA_S5_rc.tif').read(1)
        BT37=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'BT_S7.tif').read(1)
        BT11=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'BT_S8.tif').read(1)
        BT12=rasterio.open(args.inpath+os.sep+scenes[k]+os.sep+'BT_S9.tif').read(1)
